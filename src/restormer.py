#from https://github.com/leftthomas/Restormer/blob/master/model.py
...
class Restormer(pl.LightningModule):
    def __init__(self, num_blocks=[4, 6, 6, 8], num_heads=[1, 2, 4, 8], channels=[48, 96, 192, 384], num_refinement=4,
                 expansion_factor=2.66, n_channels=3, lr=0.0003, niter=300000, batch_size=10):
        super().__init__()
        # super(Restormer, self).__init__()
        self.lr = lr
        self.niter = niter
        self.batch_size = batch_size
        self.embed_conv = nn.Conv2d(n_channels, channels[0], kernel_size=3, padding=1, bias=False)
        ...
        self.output = nn.Conv2d(channels[1], n_channels, kernel_size=3, padding=1, bias=False)
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = CosineAnnealingLR(optimizer, T_max=self.niter, eta_min=1e-6)
        return {
                "optimizer": optimizer,
                "lr_scheduler": {
                    "scheduler": CosineAnnealingLR(optimizer, T_max=self.niter, eta_min=1e-6)
                },
            }
    ...
    def train_dataloader(self):
        return DataLoader(train_dataset, batch_size=self.batch_size, num_workers=31)
...
class DataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        self.training_dataset = TensorDataset(self.inputs, self.outputs)
    
    def setup(self, stage):
        self.train_ds, self.val_ds = random_split(self.training_dataset, [0.8, 0.2])

    def train_dataloader(self):
        return DataLoader(self.train_ds, batch_size=self.batch_size, num_workers=31)

    def val_dataloader(self):
        return DataLoader(self.val_ds, batch_size=self.batch_size, num_workers=31)