...
class EigenPSF:
    ...
    def create_eigenpsfs(self, method='eigen'):
        '''
        ...
        # if method == 'nmf':
        #     self.nmf_fit, self.weights, self.flattened_eigenpsfs = self.nmfpsfs(stacked_psf)
        if method == 'pca':
            self.pca, self.weights, self.flattened_eigenpsfs = self.pcapsfs(stacked_psf)
        elif method == 'eigen':
            self.weights, self.flattened_eigenpsfs = self.eigpsfs(stacked_psf)
        elif method == 'raw':
            self.weights, self.flattened_eigenpsfs = self.rawpsfs(stacked_psf)
    ...
    def nmfpsfs(self, psf_stack):
        ''' *TO BE IMPLEMENTED*
        Performs linear decomposition using NMF.
        ...
        nmf_fit : class
            The nimfa NMF class.
        weights : 2D array
            The weights associated with the NMF basis.
        flattened_nmfpsfs : 2D array
            The NMF basis.

        '''
        pass
        #ensure no negative values. Usually only due to rounding errors
        # psf_stack[psf_stack < 0] = 0
        
        # nmf = nimfa.Nmf(psf_stack.T, rank=self.n_psfs, max_iter=100, 
        #                      update='euclidean', objective='fro')
        # nmf_fit = nmf()
        # weights = np.asarray(nmf_fit.coef())
        # flattened_nmfpsfs = np.asarray(nmf_fit.basis()).T
        # return nmf_fit, weights, flattened_nmfpsfs
    
    def pcapsfs(self, psf_stack):
        '''
        Performs linear decomposition using PCA.
        ...
        pca = utils.PCA()
        pca.fit(psf_stack)
        weights = pca.components_ @ (psf_stack).T
        flattened_pcapsfs = pca.components_
        return pca, weights, flattened_pcapsfs

    def rawpsfs(self, psf_stack):
        '''
        Performs linear decomposition using one-hot-vectors.
        ...
        -------
        weights : 2D array
            The weights associated with the raw basis.
        flattened_pcapsfs : 2D array
            The raw basis. These are like [1, 0, 0, ...], [0, 1, 0, ...].
            
        '''
        
        weights = torch.eye(self.n_psfs)
        flattened_eigenpsfs = psf_stack
        return weights, flattened_eigenpsfs
    ...
    def normalizing_mask(self, n_components=None):
        ...
        mask = self.eigen_convolution(obj=torch.ones(self.obj_shape, device=self.device),
